...
use egui::{
    Align2, Color32, FontFamily, FontId, Pos2, Rect, Rounding, ScrollArea, Sense, Stroke, Vec2,
};
...
impl Default for WorkpadScreen {
    fn default() -> Self {
        Self {
...
impl WorkpadScreen {
    ...
    fn finish_edit(&mut self) {
        if let WorkpadEdit::None = self.edit {
        } else {
            println!("Send {:?}", self.edit);
            self.edit = WorkpadEdit::None;
        };
    }
    ...
    pub fn render(&mut self, ctx: &egui::Context) -> Vec<AppAction> {
        let mut result = vec![];
        egui::TopBottomPanel::top("top_panel").show(ctx, |ui| self.render_menu(ui, &mut result));
        egui::CentralPanel::default().show(ctx, |ui| self.render_pad(ui));
        result
    }

    pub fn render_menu(&mut self, ui: &mut egui::Ui, actions: &mut Vec<AppAction>) {
        egui::menu::bar(ui, |ui| {
            egui::menu::bar(ui, |ui| {
                ui.menu_button("Pad", |ui| {
                    if ui.button("Close").clicked() {
                        ui.close_menu();
                        actions.push(AppAction::CloseWorkpad);
                    }
    ...
    fn render_pad(&mut self, ui: &mut egui::Ui) {
        ui.vertical(|ui| {
            ui.heading(self.pad.name());
            ui.horizontal(|ui| {
                let _ = ui.button("a");
                let _ = ui.button("b");
                let _ = ui.button("c");
            ...
            ui.horizontal(|ui| {
                ...
                ui.push_id("sheet_name", |ui| {
                    if let WorkpadEdit::SheetName(ref mut s) = self.edit {
                ...
                ui.push_id("formula", |ui| {
                    if let WorkpadEdit::Formula(ref mut s) = self.edit {
                        let resp = ui.text_edit_singleline(s);
                        if resp.lost_focus() {
                            if ui.input(|i| i.key_pressed(egui::Key::Escape)) {
                                self.cancel_edit();
